 ...
 * @events  @on-row-click 单击行或者单击操作按钮方法
 ...
 -->
<template>
  <div :style="{width:tableWidth}" class="hs-autoTbale">
            ...
            <label @click="toggle(index,item)" v-if="!column.type">
              <span v-if="snum==iconRow()">
                ...
                <i
                  ...
                  class="ivu-icon"
                  :class="{'ivu-icon-ios-plus-outline':!item.expanded,'ivu-icon-ios-minus-outline':item.expanded }"
                ></i>
...
export default {
  ...
  watch: {
    ...
    items() {
      if (this.items) {
        ...
        this.initData(this.deepCopy(this.items), 1, null, true);
  ...
  methods: {
    ...
    // 数据处理 增加自定义属性监听
    initData(items, level, parent, isClearItems) {
      var _this = this;
      if(isClearItems){
        this.initItems = [];
      }
      ...
      items.forEach((item, index) => {
        ...
        if (item.children && item.expanded) {
          _this.initData(item.children, level + 1, item, false);
        }
...
.hs-autoTbale .table-bordered {
  border: 1px solid #ebebeb;
}
.hs-autoTbale .table > tbody > tr > td,
.hs-autoTbale .table > tbody > tr > th,
.hs-autoTbale .table > thead > tr > td,
.hs-autoTbale .table > thead > tr > th {
  border-top: 1px solid #e7eaec;
  line-height: 1.42857;
  padding: 8px;
  vertical-align: middle;
}
.hs-autoTbale .table-bordered > tbody > tr > td,
.hs-autoTbale .table-bordered > tbody > tr > th,
.hs-autoTbale .table-bordered > tfoot > tr > td,
.hs-autoTbale .table-bordered > tfoot > tr > th,
.hs-autoTbale .table-bordered > thead > tr > td,
.hs-autoTbale .table-bordered > thead > tr > th {
...
.hs-autoTbale #hl-tree-table > tbody > tr {
  background-color: #fbfbfb;
}
.hs-autoTbale #hl-tree-table > tbody > .child-tr {
  background-color: #fff;
}
.hs-autoTbale label {
  margin: 0 8px;
}
.hs-autoTbale .ms-tree-space {
  position: relative;
  top: 1px;
...
.hs-autoTbale .ms-tree-space::before {
  content: "";
}
.hs-autoTbale #hl-tree-table th > label {
  margin: 0;
}
</style>